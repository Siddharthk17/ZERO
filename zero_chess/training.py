"""Continuous self-play training utilities."""
...
from .constants import color_of, piece_type
...
class ContinuousLRScheduler:
    ...
    def lr_at(self, iteration: int) -> float:
        ...
        return self.final_lr + 0.5 * (self.initial_lr - self.final_lr) * (1 + math.cos(math.pi * iteration / 500))
...
def train_step(
    ...
    for exp in batch.experiences:
        ...
        value_targets.append(td_blended_target(exp.value, exp.td_value, config.td_lambda) + exp.reward_bonus)
...
def _auxiliary_targets(board: Board) -> tuple[float, float, float]:
    ...
    for piece in board.squares:
        if piece == "." or piece_type(piece) == "K":
            continue
        value = values[piece_type(piece)]
        if color_of(piece) == board.turn:
            own += value
        else:
            opp += value
